...
- Browser isolation: the demo exposes no file-list or folder input and runs only bundled sample details. Its only state is `localStorage["demo:cloud-exit-evidence"]`; it never reads real file details or real-data keys. **Reset demo** replaces only the demo key. **Start for real** removes it before opening the command-line setup at `/#install`.
